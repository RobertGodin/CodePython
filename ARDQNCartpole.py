...
EPSILON_DEBUT = 0.9
EPSILON_FINAL = 0.05
EPSILON_TAUX_DECROISSANCE = 200
FREQUENCE_MAJ_RNAQ_CIBLE = 10
...
def choisir_action(etat):
    ...
    global nombre_etapes_accomplies
    unif01 = random.random()
    epsilon = EPSILON_FINAL + (EPSILON_DEBUT - EPSILON_FINAL) * \
        math.exp(-1. * nombre_etapes_accomplies / EPSILON_TAUX_DECROISSANCE)
    nombre_etapes_accomplies += 1
    if unif01 > epsilon:
        with torch.no_grad():
...
for i_episode in range(NOMBRE_EPISODES):
    ...
    for t in count():
        action = choisir_action(etat) # Sélectionner et exécuter l'action
