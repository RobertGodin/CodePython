...
Version avec couche vectorisation et LSTM
"""
...
class DatasetParoles(torch.utils.data.Dataset):
    ...
    def charger_mots(self):
        ...
        text = dataframe_entrainment.iloc[0:100]['Lyric'].str.cat(sep=' ')
        return text.split(' ')
...
class Modele(nn.Module):
    """Modèle de RNR avec une couche vectorisation, suivie d'une couche LSTM et d'une couche linéaire"""
    def __init__(self, ds_paroles):
        ...
        self.lstm = nn.LSTM(input_size=self.taille_H_LSTM,hidden_size=self.taille_H_LSTM,
    ...
    def initializer_etat(self, taille_sequence):
        return (torch.zeros(self.nombre_couches_RNR, taille_sequence, self.taille_H_LSTM),
                torch.zeros(self.nombre_couches_RNR, taille_sequence, self.taille_H_LSTM))
...
def entrainer_RNR(ds_paroles, modele, taille_lot=32, epochs=10, taille_sequence=4):
    modele.train()
    ...
    for epoch in range(epochs):
        etat_H, etat_C = modele.initializer_etat(taille_sequence)

        for lot, (lot_X, lot_Y) in enumerate(dl_paroles):

            optimizeur.zero_grad()

            lot_Y_predictions, (etat_H, etat_C) = modele(lot_X, (etat_H, etat_C))
            cout = fonction_cout(lot_Y_predictions.transpose(1, 2), lot_Y)

            etat_H = etat_H.detach()
            etat_C = etat_C.detach()
...
def predire(ds, modele, debut_texte, nb_mots=100):
    ...
    modele.eval()
    etat_H, etat_C = modele.initializer_etat(len(mots))
