# -*- coding: utf-8 -*-
"""
Exemple de fonction dessiner_bot avec nom de paramètre formel différent
"""

import pygame

def dessiner_bot(f,x,y,largeur,hauteur):
    """ Dessiner un Bot. 
    
    Le Bot est inscrit dans le rectangle englobant défini par les paramètres
    (x,y,largeur et hauteur) dans une fenetre f de Pygame
    
    f : la surface de dessin
    x,y,largeur,hauteur : paramètres du rectangle englobant en pixels
    """
    ROUGE = (255,0,0)
    NOIR = (0,0,0)
    VERT = (0,255,0)    
    pygame.draw.ellipse(f, VERT, [x,y,largeur, hauteur/2]) # Dessiner la tête
    pygame.draw.rect(f, NOIR, [x+largeur/4,y+hauteur/8,largeur/10,hauteur/20]) # L'oeil gauche
    pygame.draw.rect(f, NOIR, [x+largeur*3/4-largeur/10,y+hauteur/8,largeur/10,hauteur/20]) # L'oeil droit
    pygame.draw.line(f, NOIR, [x+largeur/4,y+hauteur*3/8],[x+largeur*3/4,y+hauteur*3/8], 2) # La bouche
    pygame.draw.rect(f, ROUGE, [x,y+hauteur/2,largeur,hauteur/2]) # Le corps
