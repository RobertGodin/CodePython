...
class RNA(object):
    ...
    def traiter_un_lot(self, un_mini_lot, eta):
        ...
        self.liste_biais = [b-(eta/len(un_mini_lot))*db for (b, db) in zip(self.liste_biais, liste_dJ_db)]
        self.liste_w = [w-(eta/len(un_mini_lot))*dw  for (w, dw) in zip(self.liste_w, liste_dJ_dw)]
